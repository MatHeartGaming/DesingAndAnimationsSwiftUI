...
struct RadialLayoutView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            Rectangle()
                .fill(.gray.gradient).ignoresSafeArea()
            ...
            RoundedRectangle(cornerRadius: 4)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 4)
                        .stroke().fill(.white))
                .offset(y: -46)
                .rotationEffect(Angle.degrees(minutes))
                .shadow(radius: 5, y: 5)
                .animation(.linear(duration: 60).repeatForever(), value: minutes)
    ...
    var clockCase : some View {
        ZStack {
            Circle()
                .foregroundStyle(
                    .gray.shadow(.inner(color: .gray, radius: 30, x: 30, y: 30))
                    .shadow(.inner(color: .white.opacity(0.2), radius: 0, x: -1, y: -1))
                    .shadow(.inner(color: .black.opacity(0.4), radius: 0, x: -1, y: -1))
                )
            
            Circle()
                .foregroundStyle(
                    .white.shadow(.inner(radius: 30, x: -30, y: -30))
                    .shadow(.drop(color:.black.opacity(0.3), radius: 20, x: 30, y: 30))
                )
                .frame(width: 320)
...
struct CustomLayout : Layout {
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) {
        //subviews[0].place(at: CGPoint(x: 100, y: 110), proposal: .unspecified)
        for (index, subview) in subviews.enumerated() {
            //Position
